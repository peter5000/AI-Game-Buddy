...
from backend.app.services.games.lands.lands_interface import LandsState, LandsAction, LandsPayload
...
def test_grass_effect_with_discard_pile(lands_system: LandsSystem, initial_state: LandsState):
    player_id = "player1"
    ...
    state = lands_system.make_action(state, "player2", LandsAction(type="COUNTER", payload=LandsPayload(target=0))) # Don't counter
    ...
    assert final_state.private_state.states[player_id].hand[lv.GRASS] == 1 # 1 played, 1 returned

def test_grass_effect_empty_discard(lands_system: LandsSystem, initial_state: LandsState):
    player_id = "player1"
    initial_state.private_state.states[player_id].hand[lv.GRASS] = 1

    action = LandsAction(type="PLAY_ENERGY", payload=LandsPayload(target=lv.GRASS))
    state = lands_system.make_action(initial_state, player_id, action)
    state = lands_system.make_action(state, "player2", LandsAction(type="COUNTER", payload=LandsPayload(target=0)))
...
def test_fire_effect(lands_system: LandsSystem, initial_state: LandsState):
    ...
    state = lands_system.make_action(initial_state, player_id, action)
    state = lands_system.make_action(state, opponent_id, LandsAction(type="COUNTER", payload=LandsPayload(target=0)))
...
def test_darkness_effect_opponent_reveals(lands_system: LandsSystem, initial_state: LandsState):
    player_id = "player1"
    opponent_id = "player2"
    ...
    action = LandsAction(type="CHOOSE_TARGET", payload=LandsPayload(target=[lv.GRASS, lv.LIGHTNING, lv.FIRE]))
...
def test_counter_a_counter(lands_system: LandsSystem, initial_state: LandsState):
    ...
    opponent_id = "player2"
    initial_state.private_state.states[player_id].hand[lv.FIRE] = 1
    initial_state.private_state.states[player_id].hand[lv.WATER] = 2 # Needs 2 water to counter a counter
    initial_state.private_state.states[opponent_id].hand[lv.WATER] = 1
    initial_state.private_state.states[opponent_id].hand[lv.FIRE] = 1
...
def test_win_condition_one_of_each(lands_system: LandsSystem, initial_state: LandsState):
    player_id = "player1"
    ...
    action = LandsAction(type="CHOOSE_TARGET", payload=LandsPayload(target=0)) # Keep on top
    final_state = lands_system.make_action(state, player_id, action)
...
def test_stress_game_simulation(lands_system: LandsSystem):
    ...
    for i in range(20): # Run 20 full games
        ...
        for turn in range(150): # Max 150 turns per game
            ...
            assert valid_actions, f"No valid actions for player {current_player_id} in phase {state.phase.current} on turn {turn}"
